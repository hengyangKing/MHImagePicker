...
- (void)viewDidLoad {
    ...
    // 默认的外观，你可以在这个方法后重置
    self.oKButtonTitleColorNormal   = [TZImageManager manager].config.oKButtonTitleColorNormal;
    self.oKButtonTitleColorDisabled = [TZImageManager manager].config.oKButtonTitleColorDisabled;
...
- (instancetype)initWithMaxImagesCount:(NSInteger)maxImagesCount andConfig:(MHImagePickerAppearanceConfig *(^)())config
{
    ...
    if (self) {
        if (config) {
            [TZImageManager manager].TZImageManagerConfig(config());
        }
...
- (void)showProgressHUD {
    if (!_progressHUD) {
        ...
        _HUDLable.text = @"正在加载...";
        _HUDLable.font = [UIFont fontWithName:@"PingFangSC-Regular" size:15];
...
- (void)pushViewController:(UIViewController *)viewController animated:(BOOL)animated {
    ...
    if (self.childViewControllers.count > 0) {
        ...
        [btn setBackgroundImage:[TZImageManager manager].config.nav_back forState:UIControlStateNormal];
        [btn setBackgroundImage:[TZImageManager manager].config.nav_back_h forState:UIControlStateHighlighted];
        [btn sizeToFit];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    [cell.selectedCountButton setBackgroundImage:[TZImageManager manager].config.photo_number_bg forState:(UIControlStateNormal)];
    [cell.selectedCountButton setBackgroundImage:[TZImageManager manager].config.photo_number_bg forState:(UIControlStateHighlighted)];
